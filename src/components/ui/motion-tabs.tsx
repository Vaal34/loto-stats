...
function MotionTabs<T extends string = string>({
  defaultValue,
  value,
  onValueChange,
  children,
...
const MotionTabsTrigger = React.forwardRef<HTMLButtonElement, Omit<TabsTriggerProps, 'ref'>>((
...
function MotionTabsContent({ children, value, className, ...props }: TabsContentProps) {
  const { activeValue } = useTabs()
